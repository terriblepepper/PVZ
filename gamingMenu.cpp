...
gamingMenuDialog::gamingMenuDialog(QWidget* parent) : QDialog(parent)
{
    ...
    // 创建BGM音量滑块
    volumeLabel = new QLabel("BGM音量:"+QString::number(musicVolume)+"%", this);
    volumeSlider = new QSlider(Qt::Horizontal, this);
    volumeSlider->setRange(0, 100); // 假设音量范围是0到100
    volumeSlider->setValue(musicVolume);
    //特效音量
    itemVolumeLabel = new QLabel("特效音量:" + QString::number(itemVolume) + "%", this);
...
void gamingMenuDialog::onVolumeChanged(int volume)
{
    ...
    if (currentGameMode)
    {
        ...
        {
            musicVolume = volume;
            volumeLabel->setText("BGM音量:" + QString::number(musicVolume) + "%");
        ...
        {
            itemVolume = volume;
            itemVolumeLabel->setText("特效音量:" + QString::number(itemVolume) + "%");
        }     
    }  
}
